...
class ModelResultParser(IModelResultParser):
    ...
    def _parse_in_memory_model(self, model: tf.keras.Model, x: np.ndarray) -> tuple[float, str]:
        result = model(tf.convert_to_tensor(x, dtype=tf.float32), training=False)
    ...
    def _parse_on_disk_model(self, model: tf.keras.Model, x: np.ndarray) -> tuple[float, str]:
    ...
    def parse(self, model: tf.keras.Model, x: np.ndarray) -> Union[tuple[Literal['noise'], Literal[0]], tuple[float, str]]:
        af = self._reshape(model, self._get_af(x))
        try:
            prediction, label = self._parse_on_disk_model(model, af)
        except Exception as e:
            try:
                prediction, label = self._parse_in_memory_model(model, af)
            except Exception as e:
                self.logger.error(e)
                return 'noise', 0
