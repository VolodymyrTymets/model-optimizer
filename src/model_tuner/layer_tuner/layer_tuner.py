...
class LayerTuner(ILayerTuner):
    ...
    def _get_best_units(self, schema: IModelSchema, units: list[int], unit_steps: list[ExperimentStepModel]) -> list[
        ExperimentStepModel]:
        if len(units) == 1:
            return unit_steps
        ...
        for current_units in [low_units, high_units]:
            ...
            step = self._experiment_step.run(schema, self._details.epochs)
            steps.append(step)
        ...
        return self._get_best_units(best_schema, next_units, unit_steps)

    def get_best_settings(self, schema: IModelSchema,
        ...
        steps = [self._experiment_step.run(options_schema, self._details.epochs)]
        for activation in self._details.activation:
            for regularizer in self._details.regularizer:
                ...
                steps.append(self._experiment_step.run(options_schema, self._details.epochs))

        best_step = self._experiment_step.get_best_step(steps)
        best_schema = self._experiment_step.get_schema(best_step)
        return best_schema.layers[-1]

    def rare_tuning(self) -> Sequence[ILayerSchema]:
    ...
    def tuning(self, schema: IModelSchema,
        ...
        settings = self.get_best_settings(schema, current_layer)
        ...
        self._get_best_units(units_schema, self.units_range, units_steps)
        best_step = self._experiment_step.get_best_step(units_steps)
        best_schema = self._experiment_step.get_schema(best_step)
        best_count_of_units = best_schema.layers[-1].units
        self._logger.log(
            f"    Best units for current layer {units_schema.layers[-1].type}: {best_count_of_units} found on step {best_step.step}",
            color="green")
